  ...
  if($type == "create"){
    ...
    if($player_other && ($player_other->id != $user->id)){
      ...
      if((!$current_move) || $current_move->has_expired() || $current_move->has_ended()){
        ...
        // create a new move for the next eligible player
      ...
      else if(($current_move->round === 4) || (isset($game->has_been_won))){
        // game ends at round 3
        $game->select_winner();
        $game->plan_next_move();
    ...
    if(($description) && ($current_move->user_id === $user->id)){
      $current_move->close($type, $description);
    }
  }
